...
def get_ratio(xmin, ymin, xmax, ymax, label, img_path):
    ...
    xdiff = int((xmax - xmin) // 5)
    ydiff = int((ymax - ymin) // 5)
    for i in range(5):
        for j in range(5):
...
'''
this function generate 25 ratio in each bounding box and add them to xml files
'''
    ...
    for f in os.listdir(anno_path):
        ...
        for obj in annot.findall('object'):
            ...
            if os.path.isfile(segment_path):
                ...
                # root = annot.getroot()
                # pretty_xml(root, '\t', '\n')
                new_path = image_xml_path.replace('Annotations-all', 'Annotations-25')
