...
def get_ratio(xmin, ymin, xmax, ymax, label, img_path):
    ...
    xdiff = int((xmax - xmin) // 4)
    ydiff = int((ymax - ymin) // 4)
    for i in range(4):
        for j in range(4):
...
'''
this function generate 16 ratio in each bounding box and add them to xml files
'''

def edit_annotaions():
    anno_path = os.path.join(data_path, 'VOCtest-2007', 'Annotations-all')

    for f in os.listdir(anno_path):
        id = f.split('.')[0]
        image_xml_path = os.path.join(anno_path, "{}.xml".format(id))
        segment_path = os.path.join(data_path, 'VOCtest-2007', 'SegmentationObject', "{}.png".format(id))
        ...
        for obj in annot.findall('object'):
            ...
            if os.path.isfile(segment_path):
                ratio = get_ratio(xmin, ymin, xmax, ymax, label, segment_path)
                ...
                # root = annot.getroot()
                # pretty_xml(root, '\t', '\n')
                new_path = image_xml_path.replace('Annotations-all', 'Annotations')
