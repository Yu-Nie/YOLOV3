...
class Trainer(object):
    ...
    def __save_model_weights(self, epoch, mAP):
        if mAP > self.best_mAP:
            self.best_mAP = mAP
        best_weight = os.path.join(os.path.split(self.weight_path)[0], "best_all.pt")
    ...
    def train(self):
        ...
        for epoch in range(self.start_epoch, self.epochs):
            ...
            for i, (imgs, label_sbbox, label_mbbox, label_lbbox, sbboxes, mbboxes, lbboxes) in enumerate(
                ...
                loss, loss_giou, loss_conf, loss_cls, loss_ratios = self.criterion(p, p_d, label_sbbox, label_mbbox,
                ...
                loss_items = torch.tensor([loss_giou, loss_conf, loss_cls, loss_ratios, loss])
                mloss = (mloss * i + loss_items) / (i + 1)

                # Print batch results
                if i % 10 == 0:
                    s = (
                            'Epoch:[ %d | %d ]    Batch:[ %d | %d ]    loss_giou: %.4f    loss_conf: %.4f    loss_cls: %.4f    loss_ratio: %.4f     loss: %.4f    '
                            'lr: %g') % (
...
if __name__ == "__main__":
    ...
    parser.add_argument('--weight_path', type=str, default=r"C:\Users\mrcry\Documents\weight\darknet53_448.weights", help='weight file path')
